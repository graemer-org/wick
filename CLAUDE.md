...
- `action.yml` — composite action, `mode: report` (sticky PR comment via `scripts/pr-comment.mjs`), `mode: reconcile` (remap stamps after squash/rebase merge), and `mode: stamp` (capture a CI run's cost from the runner's Claude Code transcripts). `stamp` takes `base-sha` (HEAD before the agent ran; required — an omitted `base-sha` skips capture rather than blind-stamp and risk double-counting on re-runs) and optional `comment-issue`: when HEAD advanced it stamps + pushes notes (`wick hook post-commit` then `pre-push`), and when it did not (a no-commit run has nothing to anchor a note to and must not pollute the pre-existing HEAD) it computes the run's cost via `wick cost` (whose rendered line is the single source of the token/cost formatting) and, **only if `comment-issue` is set**, posts it as a one-line comment (otherwise just logs it); a genuinely no-op run (0 sessions) skips the comment. Wired into all three Claude-running workflows — `claude.yml` (`@claude` responder, may commit → `base-sha` stamp/comment), and `claude-pr-review.yml` / `claude-issue-triage.yml` (`comment-only: true` → always comment, never stamp). `comment-only` exists because `claude-code-action` moves `HEAD` (it checks out the PR head), which false-triggers the `HEAD != base-sha` "commit landed" branch — review/triage never make a dev commit, so they skip detection entirely.
