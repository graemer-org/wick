...
- `src/config.ts` — repo config from `.wick/config.json` (currently: per-PR budget with warn fraction). Tolerant loader, never throws. Budgets attach only to branch-scoped ranges (not full-history "HEAD"). Budgets inform, they never block — Patrick rejected an enforce/fail mode because an over-budget PR can't get cheaper, so a failing check would dead-end the PR; over-budget surfaces as a CI warning annotation instead. Don't reintroduce enforcement.
