...
- `action.yml` — composite action, `mode: report` (sticky PR comment via `scripts/pr-comment.mjs`), `mode: reconcile` (remap stamps after squash/rebase merge), and `mode: stamp` (capture a CI run's cost from the runner's Claude Code transcripts). `stamp` takes `base-sha` (HEAD before the agent ran) and optional `comment-issue`: when HEAD advanced it stamps + pushes notes (`wick hook post-commit` then `pre-push`), and when it did not it reports the run's cost via `wick cost --json` as a one-line comment (a no-commit run has no commit to anchor a note to and must not pollute the pre-existing HEAD). Wired into `.github/workflows/claude.yml` after the `@claude` step.
