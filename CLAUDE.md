...
- `src/config.ts` — repo config from `.wick/config.json` (currently: per-PR budget with warn fraction + enforce flag). Tolerant loader, never throws. Budgets attach only to branch-scoped ranges (not full-history "HEAD"), status `unknown` (unpriced models) never enforces.
